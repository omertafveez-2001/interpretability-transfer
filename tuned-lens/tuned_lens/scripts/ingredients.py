...
_MAMBA_TOKENIZER = "EleutherAI/gpt-neox-20b"
_RWKV_TOKENIZER  = "EleutherAI/gpt-neox-20b"
...
class Model:
    ...
    def _is_mamba(self) -> bool:
        return "mamba" in self.name.lower() and _mamba_available

    def _is_rwkv(self) -> bool:
        return "rwkv" in self.name.lower()

    def load_tokenizer(self, must_use_cache: bool = False) -> PreTrainedTokenizerBase:
        """Load the tokenizer from huggingface hub."""
        # Mamba and RWKV-v4 pile models use the GPT-NeoX tokenizer
        if self._is_mamba():
            tokenizer_name = self.tokenizer or _MAMBA_TOKENIZER
        elif self._is_rwkv():
            tokenizer_name = self.tokenizer or _RWKV_TOKENIZER
        else:
            tokenizer_name = self.tokenizer or self.name
    ...
    def load(
        ...
        # Mamba requires mamba_ssm loader — not compatible with AutoModelForCausalLM
        if self._is_mamba():
            model = _MambaLMHeadModel.from_pretrained(self.name)
            if device is not None:
                model = model.to(device)
            model.eval()
            model.requires_grad_(False)
            # Patch config so TunedLens can read d_model and num_hidden_layers
            model.config.hidden_size = model.config.d_model
            model.config.num_hidden_layers = model.config.n_layer
            model.config.name_or_path = self.name
            return model, self.load_tokenizer(must_use_cache=must_use_cache)

        # RWKV must not use device_map="auto" (needs to be fully on one device)
        if self._is_rwkv():
            with handle_name_conflicts():
