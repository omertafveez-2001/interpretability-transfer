...
class Train:
    ...
    def _forward_collect_hidden(self, model, batch):
        """Run a forward pass and return (final_logits, hidden_states_tuple).

        For standard HF models uses output_hidden_states=True.
        For Mamba (which lacks that kwarg) captures states via forward hooks.
        """
        is_mamba = self.model._is_mamba()
        if not is_mamba:
            output = model(**batch, output_hidden_states=True)
            return output.logits, output.hidden_states[:-1]

        # Mamba path: collect hidden states via hooks
        collected: list[th.Tensor] = []
        handles = []
        ...
        handles.append(model.backbone.embedding.register_forward_hook(_make_hook()))
        # outputs of layers 0 .. N-2 → inputs to layers 1 .. N-1
        for layer in list(model.backbone.layers)[:-1]:
        ...
        return output.logits, tuple(collected)
    ...
    def execute(self):
        ...
        for batch_idx, batch in zip(t, state.dataloader):
            ...
            with th.no_grad():
                ...
                final_logits, hidden_states = self._forward_collect_hidden(model, batch)
