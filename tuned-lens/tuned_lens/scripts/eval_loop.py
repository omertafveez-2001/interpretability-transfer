...
class Eval:
    ...
    def _forward_collect_hidden(self, model, batch):
        """Forward pass returning (logits, hidden_states_tuple).

        Uses output_hidden_states=True for HF models; forward hooks for Mamba.
        """
        import torch as th
        if not self.model._is_mamba():
    ...
    def execute(self):
        ...
        for batch in pbar:
            ...
            final_lps, hidden_states = self._forward_collect_hidden(model, batch)
            final_lps = final_lps.log_softmax(dim=-1)

            final_probs = final_lps.exp()
            assert not th.isnan(final_lps).any(), "Logits are NaN"
